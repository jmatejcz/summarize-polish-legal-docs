...
class QLoRADataset(Dataset):
    """Dataset for QLoRA training with proper chat template format"""
    ...
    def __getitem__(self, idx):
        ...
        # Use exact same format as inference - this is crucial!
        messages = [
        ...
        # Create labels - mask prompt tokens so loss is only computed on assistant response
...
class QLoRAPipeline:
    ...
    def _setup_model_and_tokenizer(self):
        ...
        logger.info(f"Loading model with 4-bit quantization: {self.model_name}")
    ...
    def _prepare_datasets(self, val_split=0.1, max_length=3072):
        ...
        train_data = create_train_data_for_prompt_tuning(
            ...
            max_len=max_length,  # Adjust based on your average document length
        )
    ...
    def generate_summary(
        ...
        """Generate a summary using the QLoRA-trained model"""
        messages = [
            {"role": "system", "content": system_prompt},
            {"role": "user", "content": f"Streść poniższy dokument:\n{text}"},
        ]
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="QLoRA Fine-tuning Pipeline for Language Models"
    )
    ...
    parser.add_argument(
        "--max_length", type=int, default=3072, help="Maximum sequence length"
    )
...
def main():
    ...
    logger.info("Starting QLoRA Fine-tuning Pipeline with the following configuration:")
